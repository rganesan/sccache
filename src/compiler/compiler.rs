    ...
    T: CommandCreatorSync,
{
    ...
    let do_dist_compile = async move {
        ...
        if retry_enabled && exit_code != 0 {
            ...
            return Err(crate::errors::RemoteCompileFailure { exit_code }.into());
        }
    ...
    do_dist_compile
        .or_else(move |e| async move {
                ...
                e.downcast_ref::<lru_disk_cache::Error>()
            {
                ...
                if let Some(remote_err) = e.downcast_ref::<crate::errors::RemoteCompileFailure>() {
                    warn!(
                        "[{}]: Remote compilation failed with exit code {}, will retry locally",
                        out_pretty2, remote_err.exit_code
                    );
                } else {
